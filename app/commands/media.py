...
def _display_youtube_video_info(
    info, for_download=False, quality=None, output_dir=None
):
    ...
    if not for_download and duration:
        ...
        duration_str = (
            f"{hours}h {mins}m {secs}s" if hours > 0 else f"{mins}m {secs}s"
        )
...
@click.option(
    ...
    default=1200.0,
    show_default=True,
    help="Maximum length of each chunk (in seconds)",
)
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Path to save the converted MP3 file",
)
...
@click.option(
    "--quality",
    default="best",
    help="Video quality to download (e.g., '360p' or 'best'). Use '--list-formats' to see options.",
)
@click.option(
    "--list-formats",
    is_flag=True,
    help="List available formats for the video instead of downloading.",
)
def download_youtube_video(youtube_url, output_dir, quality, list_formats):
    ...
    if not youtube_url:
        click.echo(
            "Please provide a YouTube URL or use the --help flag for more options."
        )
        return
            ...
            if formats:
                ...
                for f in formats:
                    ...
                    if filesize:
                        ...
                        size_in_mb = (bitrate * 1000 / 8 * duration) / (
                            1024 * 1024
                        )
                ...
                click.echo("\n" + "-" * 105)
                click.echo(
                    "You can use the 'ID' or RESOLUTION (e.g., '360p') with the 'download-youtube-video' command."
                )
                click.echo("\nExamples:")
                click.echo(
                    "  `tstbtc media download-youtube-video <url> --quality 360p`"
                )
                click.echo(
                    "  `tstbtc media download-youtube-video <url> --quality 232`"
                )
            else:
                logger.error("Could not retrieve formats.")
        else:
            _display_youtube_video_info(
                info, for_download=True, quality=quality, output_dir=output_dir
            )

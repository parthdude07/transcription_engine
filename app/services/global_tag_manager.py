...
class GlobalTagManager:
    ...
    def __init__(self, metadata_dir=None):
        self.metadata_dir = (
            metadata_dir or settings.TSTBTC_METADATA_DIR or "metadata/"
        )
        self.dict_file = os.path.join(
            self.metadata_dir, "global_tag_dictionary.json"
        )
    ...
    def _load_dictionary(self) -> dict[str, Any]:
        ...
        if os.path.exists(self.dict_file):
            try:
                with open(self.dict_file, encoding="utf-8") as f:
                    return json.load(f)
            except (OSError, json.JSONDecodeError) as e:
                logger.warning(
                    f"Failed to load global tag dictionary: {e}. Creating new one."
                )
    ...
    def _create_new_dictionary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tags": {},
            "technical_terms": [],
            "speaker_context": {"common_speakers": [], "expertise_areas": []},
            "project_names": [],
            "categories": {},
            "video_count": 0,
            "common_words": {},
    ...
    def _save_dictionary(self):
        ...
        try:
            ...
            self.tag_dict["last_updated"] = datetime.now(
                timezone.utc
            ).isoformat()
    ...
    def update_from_transcript(self, transcript):
        ...
        try:
            ...
            youtube_tags = (
                youtube_metadata.get("tags", []) if youtube_metadata else []
            )
            ...
            self.tag_dict["video_count"] = (
                self.tag_dict.get("video_count", 0) + 1
            )
            ...
            logger.info(
                f"Updated global tag dictionary with transcript: {title}"
            )
    ...
    def _update_tag_entry(self, tag: str):
        ...
        if tag_lower in tags_dict:
            ...
            tags_dict[tag_lower]["last_seen"] = datetime.now(
                timezone.utc
            ).isoformat()
    ...
    def _update_speaker_context(self, speaker: str):
        ...
        if speaker not in speaker_context["common_speakers"]:
            ...
            if len(speaker_context["common_speakers"]) > 50:
                speaker_context["common_speakers"] = speaker_context[
                    "common_speakers"
                ][-50:]
    ...
    def _update_category_frequency(self, category: str):
        ...
        categories_dict[category_lower] = (
            categories_dict.get(category_lower, 0) + 1
        )
        self.tag_dict["categories"] = categories_dict

    def _extract_technical_terms_dynamically(
        self, text_content: str, tags: list[str]
    ):
        ...
        is_bitcoin_content = any(
            indicator in text_content
            or any(indicator in tag.lower() for tag in tags)
            ...
            for tag in tags:
                if tag and len(tag) > 3 and not tag.isdigit():
                    ...
                    if (
                        ...
                        or tag_lower.startswith(("op_", "bip", "bolt"))
                        or tag_lower
                        in ["taproot", "segwit", "multisig", "htlc"]
                    ) and tag_lower not in technical_terms:
                        technical_terms.append(tag_lower)

        self.tag_dict["technical_terms"] = technical_terms

    def _identify_project_names_dynamically(
        self, text_content: str, tags: list[str]
    ):
        ...
        for tag in tags:
            if tag and len(tag) > 2 and (
                tag[0].isupper()
                or "core" in tag.lower()
                or "lightning" in tag.lower()
                or any(
                    pattern in tag.lower()
                    for pattern in ["btc", "lightning", "wallet", "pay"]
                )
            ) and tag not in project_names:
    ...
    def _update_expertise_areas(self, tags_and_categories: list[str]):
        ...
        expertise_areas = self.tag_dict.get("speaker_context", {}).get(
            "expertise_areas", []
        )
    ...
    def get_correction_context(self) -> dict[str, Any]:
        ...
        frequent_tags = sorted(
            tags_dict.items(), key=lambda x: x[1]["frequency"], reverse=True
        )[:30]
        ...
        return {
            ...
            "technical_terms": self.tag_dict.get("technical_terms", []),
            "project_names": self.tag_dict.get("project_names", []),
            "common_speakers": self.tag_dict.get("speaker_context", {}).get(
                "common_speakers", []
            )[:20],
            ...
            for category, _ in top_categories:
                if category in ["development", "technical"]:
                    context_parts.append(
                        "Bitcoin development and technical implementation"
                    )
                    ...
                    context_parts.append(
                        "Bitcoin conferences and presentations"
                    )

        for area in expertise_areas[:3]:
            if (
                area == "payments"
                and "payment" not in " ".join(context_parts).lower()
            ):
                context_parts.append("Bitcoin payments and Lightning Network")
            elif (
                area == "security"
                and "security" not in " ".join(context_parts).lower()
            ):
                context_parts.append("Bitcoin security and cryptography")
            elif (
                area == "mining"
                and "mining" not in " ".join(context_parts).lower()
            ):
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_tag_occurrences": sum(
                data["frequency"] for data in tags_dict.values()
            ),
            "technical_terms_count": len(
                self.tag_dict.get("technical_terms", [])
            ),
            "project_names_count": len(self.tag_dict.get("project_names", [])),
            "speakers_count": len(
                self.tag_dict.get("speaker_context", {}).get(
                    "common_speakers", []
                )
            ),
            "categories_count": len(categories_dict),
            "expertise_areas_count": len(
                self.tag_dict.get("speaker_context", {}).get(
                    "expertise_areas", []
                )
            ),

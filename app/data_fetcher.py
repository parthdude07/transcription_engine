...
class DataFetcher:
    ...
    def fetch_json(
        self,
        name: Literal["status", "sources", "directories"],
        cache: bool = False,
    ):
        """Fetches JSON data from a configured URL or local cache"""
        cached_file_path = (
            os.path.join(self.cache_dir, f"{name}.json")
            if self.cache_dir
            else None
        )
    ...
    def get_existing_media(self) -> dict[str, bool]:
        ...
        return {
            value: True for value in data.get("existing", {}).get("media", [])
        }
    ...
    def get_sources(
        ...
        if transcription_coverage != "none":
            filtered_data = [
                ...
                if source.get("transcription_coverage")
